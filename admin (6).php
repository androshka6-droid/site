...
function applyThemeToMainSite($themeVars, $fixTemplateOnce = true) {
    $styleFile = 'style.css';
    if (!file_exists($styleFile)) return false;
    ...
    if ($fixTemplateOnce && file_exists('template.html')) {
        ...
        if (strpos($tpl, 'href="../styles.css"') !== false) {
            $tpl = str_replace('href="../styles.css"', 'href="../style.css"', $tpl);
        }
...
function generateCarPage($carData) {
    ...
    foreach ($images as $i => $img) {
        $thumbs .= "<div class='thumb " . ($i == 0 ? "active" : "") . "' data-index='$i'>\n";
        $thumbs .= "  <img src='../assets/images/$img' alt='" . htmlspecialchars($title) . "'>\n";
        $thumbs .= "</div>\n";
    }
    ...
    foreach ($descLines as $line) {
        $line = trim($line);
        if (!empty($line)) {
            $descriptionList .= "<li>" . htmlspecialchars($line) . "</li>\n";
        }
    }

    // Спецификации
    $specs = "<div><strong>Year</strong></div><div>$year</div>\n";
    $specs .= "<div><strong>Make</strong></div><div>$make</div>\n";
    $specs .= "<div><strong>Model</strong></div><div>$model</div>\n";
    $specs .= "<div><strong>Mileage</strong></div><div>" . number_format($mileage, 0, '.', ',') . " miles</div>\n";
    $specs .= "<div><strong>VIN</strong></div><div>{$carData['vin']}</div>\n";
    $specs .= "<div><strong>Engine</strong></div><div>{$carData['engine']}</div>\n";
    $specs .= "<div><strong>Transmission</strong></div><div>{$carData['transmission']}</div>\n";
    $specs .= "<div><strong>Exterior Color</strong></div><div>{$carData['exterior_color']}</div>\n";
    $specs .= "<div><strong>Interior Color</strong></div><div>{$carData['interior_color']}</div>\n";

    $shortdesc = htmlspecialchars(mb_substr($description, 0, 200)) . '...';
    ...
    $replace = [
        ...
        '{{MAINIMAGE}}' => htmlspecialchars($mainImage),
        '{{IMAGES_THUMBS}}' => $thumbs,
        '{{DESCRIPTION_LIST}}' => $descriptionList,
        '{{SHORTDESC}}' => "<strong class='price'>\$$priceFormatted</strong><p>$shortdesc</p>",
        '{{SHORTDESC_TEXT}}' => $shortdesc,
        '{{SPECS}}' => $specs,
        '{{SLUG}}' => $slug
    ];
